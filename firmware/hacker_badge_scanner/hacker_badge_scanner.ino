...
// =============================================================================
// Hacker Badge Mobile RF Scanner Firmware
// Hardware: 2026 Hacker Badge (ESP32-C3-MINI-1-N4, 4MB Flash, USB-Serial/JTAG)
// Role: MOBILE RF Sensing Node (sensor_id: badge_01)
// Architecture:
//   - Passively scans 2.4 GHz channels for all nearby 802.11 beacons / frames
//   - Reports BSSID, SSID, RSSI, channel, and authmode to /api/ingest
//   - Classified by backend as MOBILE (contributes corroboration, not trilateration anchor)
// =============================================================================
...
String escapeJsonString(const String& input) {
  ...
  for (unsigned int i = 0; i < input.length(); i++) {
    ...
    if (c == '"') {
      ...
      // Preserve all printable ASCII and valid UTF-8 multibyte characters (128-255)
      output += c;
    }
  }
  return output;
...
void setup() {
  ...
  Serial.println();
  Serial.println("==================================================");
  Serial.println("[HACKER-BADGE] ESP32-C3 Mobile RF Threat Scanner");
  Serial.println("Hardware: 2026 Hacker Badge (ESP32-C3-MINI-1-N4)");
  Serial.println("Node Role: MOBILE Sensor Node");
  Serial.printf("Config: Sensor ID='%s' | Backend=http://%s:%d/api/ingest\n",
  ...
  Serial.printf("[HW_IDENTITY] Active SENSOR_ID: %s (MOBILE)\n", POD_ID);
}

void loop() {
  scanCycle++;
  Serial.println("--------------------------------------------------");
  Serial.printf("[CYCLE #%lu] Badge passive RF scan starting... (Heap: %u bytes)\n",
                scanCycle, ESP.getFreeHeap());

  // 1. Passive Wi-Fi scan across all 2.4 GHz channels
  ...
  // 2. Format JSON according to observation schema
  String jsonPayload = "";
  ...
  for (int16_t i = 0; i < n; i++) {
    ...
    if (i < 3 || i == n - 1) {
      ...
      Serial.println("  ... [additional APs omitted from serial output] ...");
    }
  }
  jsonPayload += "]}";

  // Free scan results memory before establishing connection
  WiFi.scanDelete();
  ...
  if (httpCode == 200) {
    ...
    if (responseBody.indexOf("\"status\":\"accepted\"") >= 0) {
      ...
      Serial.printf("[HW_IDENTITY] Node: %s (MOBILE) | STA MAC: %s\n", POD_ID, WiFi.macAddress().c_str());
    } else {
